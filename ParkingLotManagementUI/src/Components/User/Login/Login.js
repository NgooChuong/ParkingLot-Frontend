...
import APIs, { authApi, endpoints, identityApi, UserApi } from "../../../configs/APIs";
...
const SignUpForm = ({ setIsSignIn, isSignIn }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (user.avatar) {
      ...
      try {
        let res = await identityApi().post(endpoints["users"], formData, {
          headers: {
...
const SignInForm = () => {
  ...
  const loginUser = async (event) => {
    ...
    try {
      ...
      let res = await identityApi().post(endpoints["login"], {
        username: username,
        password: password,
      });
      console.log("res", res.data.result.token);
      // setTest(res.data.access_token)
      cookies.save("access-token", res.data.result.token);
      setTimeout(async () => {
        // setLoading(false);
        let userRes = await authApi(res.data.result.token).get(endpoints["current-user"]);
  ...
  const handleLogin = async (username) => {
    ...
    try {
      console.log("username", username);
      // Gửi yêu cầu HTTP sử dụng fetch hoặc axios
      let res = await identityApi().post(endpoints["LoginGG"], {
